...
if [ -b /dev/${DEV_NAME} ]; then
    ...
    if access_ok "/sys/block/${DEV_NAME}/queue/scheduler";then
    ...
    if access_ok "/sys/block/${DEV_NAME}/queue/nomerges";then
    ...
    if access_ok "/sys/block/${DEV_NAME}/queue/nr_requests";then
