...
SPDK_ISCSI_APP="${SPDK_APP_DIR}/${SPDK_APP_NAME} -c ${SPDK_ROOT_DIR}/iscsi.conf.in -m 0XFF --shm-id=${HM_SHM_ID} --logflag iscsi --logflag scsi --logflag bdev --logflag bdev_malloc &> ${SPDK_LOG_DIR}/${SPDK_APP_NAME}.log"
