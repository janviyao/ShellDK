...
do
    ...
    do
        ...
        do
            ...
            combine_str="${bdev_name}"
            if ! array_has "${create_bdev_array[*]}" "${combine_str}";then
                arr_idx=${#create_bdev_array[*]}
                create_bdev_array[${arr_idx}]="${combine_str}"
            fi
        done
...
if [[ "${op_mode}" == "create_portal_group" ]];then
    ...
    do
        ...
        echo_info "iscsi_create_portal_group ${item}"
        ${ISCSI_APP_UCTRL} iscsi_create_portal_group ${item}
        if [ $? -ne 0 ];then
...
if [[ "${op_mode}" == "create_initiator_group" ]];then
    ...
    do
        ...
        echo_info "iscsi_create_initiator_group ${item}"
        ${ISCSI_APP_UCTRL} iscsi_create_initiator_group ${item}
        if [ $? -ne 0 ];then
...
if [[ "${op_mode}" == "create_bdev" ]];then
    ...
    do
        if [[ ${BDEV_TYPE,,} == "malloc" ]];then
            echo_info "bdev_malloc_create -b ${item} ${BDEV_SIZE_MB} 4096"
            ${ISCSI_APP_UCTRL} bdev_malloc_create -b ${item} ${BDEV_SIZE_MB} 4096
        elif [[ ${BDEV_TYPE,,} == "null" ]];then
            echo_info "bdev_null_create ${item} ${BDEV_SIZE_MB} 4096"
            ${ISCSI_APP_UCTRL} bdev_null_create ${item} ${BDEV_SIZE_MB} 4096
...
if [[ "${op_mode}" == "create_target_node" ]];then
    ...
    do
        ...
        echo_info "iscsi_create_target_node ${item}"
        ${ISCSI_APP_UCTRL} iscsi_create_target_node ${item}
        if [ $? -ne 0 ];then
        ...
        if [[ ${BDEV_TYPE,,} == "cstor" ]];then
            ...
            ${ISCSI_APP_UCTRL} iscsi_target_node_set_vcns ${target_node}
            if [ $? -ne 0 ];then
