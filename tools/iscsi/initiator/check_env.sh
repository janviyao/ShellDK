...
can_access "/usr/sbin/iscsid" || { cd ${MY_VIM_DIR}/deps; install_from_rpm "iscsi-initiator-utils-.+\.rpm" true; }
can_access "sg_raw"           || { cd ${MY_VIM_DIR}/deps; install_from_rpm "sg3_utils-.+\.rpm" true; }

if math_bool "${ISCSI_MULTIPATH_ON}" && math_expr_if "${ISCSI_SESSION_NR} > 1";then
    can_access "/usr/sbin/dmsetup"            || { cd ${MY_VIM_DIR}/deps; install_from_rpm "device-mapper-1.+\.rpm" true; }
    can_access "/usr/lib64/libdevmapper.so.*" || { cd ${MY_VIM_DIR}/deps; install_from_rpm "device-mapper-libs-.+\.rpm" true; }
    can_access "/usr/lib64/libmultipath.so.*" || { cd ${MY_VIM_DIR}/deps; install_from_rpm "device-mapper-multipath-libs-.+\.rpm" true; }
    can_access "/usr/sbin/multipathd"         || { cd ${MY_VIM_DIR}/deps; install_from_rpm "device-mapper-multipath-.+\.rpm" true; }
    can_access "/usr/lib64/libaio.so.*"       || { cd ${MY_VIM_DIR}/deps; install_from_rpm "libaio-.+\.rpm" true; }
fi
