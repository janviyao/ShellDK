...
function kvconf_has_key
{
    ...
    if grep -P "^\s*${key_str}\s*=" ${kv_file} &>/dev/null;then
        return 0
    else
        return 1
    fi
...
function kvconf_set
{
    ...
    if kvconf_has_key "${kv_file}" "${key_str}";then
        ...
        sed -i "s/${key_str}=.\+/${key_str}=${val_str}/g" ${kv_file}
    else
        sed -i "\$a\\${key_str}=${val_str}" ${kv_file}
    fi
...
function kvconf_get
{
    ...
    local val_str=$(grep -P "^\s*${key_str}\s*=" ${kv_file})
    if [ -n "${val_str}" ];then
        val_str=$(replace_regex "${val_str}" "^\s*${key_str}\s*=" "")
...
function kvconf_insert
{
    ...
    sed -i "${line_nr}i\\${key_str}=${val_str}" ${kv_file}
...
function kvconf_line_nr
{
    ...
    key_str=$(replace_regex "${key_str}" "/" "\/")
    local line_nr=$(sed -n "/^\s*${key_str}\s*=/{=;q;}" ${kv_file})
    echo "${line_nr}"
    return 0
}
