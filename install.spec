make-3.82;           install_check 'make' 'make-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'make-.+\.rpm' true
...
#gcc;                install_check 'gcc' 'gcc-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'gcc-.+\.rpm' true
...
ag;                  install_check 'ag' 'the_silver_searcher-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'the_silver_searcher-.+\.rpm' true
...
ack-grep;            install_check 'ack-grep' 'ack-2.14-single-file' false;cd ${MY_VIM_DIR}/deps;cp -f ack-* ${LOCAL_BIN_DIR}/ack-grep;chmod 777 ${LOCAL_BIN_DIR}/ack-grep
...
iperf3;              install_check 'iperf3' 'iperf3-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'iperf3-.+\.rpm' true
iproute;             ! can_access 'ss';cd ${MY_VIM_DIR}/deps;install_from_rpm 'iproute-.+\.rpm' true
rsync;               install_check 'rsync' 'rsync-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'rsync-.+\.rpm' true
nmap-ncat;           install_check 'nc' 'nmap-ncat-.+\.rpm' true;cd ${MY_VIM_DIR}/deps;install_from_rpm 'nmap-ncat-.+\.rpm' true
...
readline;            ! can_access '/lib64/libreadline.so.6';cd ${MY_VIM_DIR}/deps;install_from_rpm 'readline-.+\.rpm' true
readline-devel;      ! can_access '/usr/include/readline';cd ${MY_VIM_DIR}/deps;install_from_rpm 'readline-devel-.+\.rpm' true
compat-openssl10;    ! can_access '/usr/lib64/libssl.so.10';cd ${MY_VIM_DIR}/deps;install_from_rpm 'compat-openssl10-.+\.rpm' true
python-devel;        ! can_access '/usr/bin/python-config';cd ${MY_VIM_DIR}/deps;install_from_rpm 'python-devel.+\.rpm' true
python-libs;         ! can_access '/usr/lib64/libpython2.7.so.1.0';cd ${MY_VIM_DIR}/deps;install_from_rpm 'python-libs.+\.rpm' true
python3-devel;       ! can_access '/usr/bin/python3-config';cd ${MY_VIM_DIR}/deps;install_from_rpm 'python3-devel.+\.rpm' true
python3-libs;        ! can_access '/usr/lib64/libpython3.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'python3-libs-.+\.rpm' true
xz-libs;             ! can_access '/usr/lib64/liblzma.so.5';cd ${MY_VIM_DIR}/deps;install_from_rpm 'xz-libs.+\.rpm' true
xz-devel;            ! can_access '/usr/lib64/liblzma.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'xz-devel.+\.rpm' true
libiconv;            ! can_access '/usr/libiconv/lib64/libiconv.so.2';cd ${MY_VIM_DIR}/deps;install_from_rpm 'libiconv-1.+\.rpm' true
libiconv-devel;      ! can_access '/usr/libiconv/lib64/libiconv.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'libiconv-devel.+\.rpm' true
pcre;                ! can_access '/usr/lib64/libpcre.so.1';cd ${MY_VIM_DIR}/deps;install_from_rpm 'pcre-8.+\.rpm' true
pcre-devel;          ! can_access '/usr/lib64/libpcre.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'pcre-devel.+\.rpm' true
#pcre-cpp;           ! can_access '/usr/lib64/libpcrecpp.so.0';cd ${MY_VIM_DIR}/deps;install_from_rpm 'pcre-cpp.+\.rpm' true
#cpre-utf16;         ! can_access '/usr/lib64/libpcre16.so.0';cd ${MY_VIM_DIR}/deps;install_from_rpm 'pcre-utf16.+\.rpm' true
#pcre-utf32;         ! can_access '/usr/lib64/libpcre32.so.0';cd ${MY_VIM_DIR}/deps;install_from_rpm 'pcre-utf32.+\.rpm' true
ncurses-base;        ! can_access '/usr/share/terminfo/x/xterm';cd ${MY_VIM_DIR}/deps;install_from_rpm 'ncurses-base.+\.rpm' true
ncurses-libs;        ! can_access '/usr/lib64/libncurses.so.5';cd ${MY_VIM_DIR}/deps;install_from_rpm 'ncurses-libs.+\.rpm' true
ncurses-devel;       ! can_access '/usr/lib64/libncurses.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'ncurses-devel.+\.rpm' true
zlib;                ! can_access '/usr/lib64/libz.so.1';cd ${MY_VIM_DIR}/deps;install_from_rpm 'zlib-1.+\.rpm' true
zlib-devel;          ! can_access '/usr/lib64/libz.so';cd ${MY_VIM_DIR}/deps;install_from_rpm 'zlib-devel.+\.rpm' true
