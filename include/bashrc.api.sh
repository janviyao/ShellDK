...
readonly BASH_WORK_DIR="${GBL_USER_DIR}/bash.${ROOT_PID}"
...
alias mywget='function mywget { if [ $# -eq 2 ];then local fname=$2; elif [ $# -eq 1 ];then local fname=$(string_gensub "$1" "(?<=\/)[^\/]+$"); else return 1; fi; wget "$1" -O ${fname}; return $?; }; mywget'
